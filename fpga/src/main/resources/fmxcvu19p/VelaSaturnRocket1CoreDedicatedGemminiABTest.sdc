# =============================================================================
# Project-specific constraints for VelaSaturnRocket1CoreDedicatedGemmini
# XCVU19P FMXCVU19P Board
# 1x RocketCore + 1x Saturn (vLen=256, dLen=256) + 1x Gemmini
#
...
# -----------------------------------------------------------------------------
# PBlock for Gemmini only — NO Saturn pblock for dLen=256
#
# Why no Saturn pblock:
#   dLen=256 VRF contains ~25600 distributed RAMs (2× dLen=128).
#   The VRF read buses are 256 bits wide and fan out from 4 banks to 8 FMA
#   units.  Any wide horizontal pblock causes these buses to span many
#   columns, saturating WW routing tracks (measured: West 64×64 area at
#   100.954% congestion with X0Y0:X7Y7 pblock).
#   Any narrow pblock squeezes the VRF into high-density vertical strips.
#   Vivado's AltSpreadLogic placement directive handles this better than
#   manual pblocks — it spreads logic proportionally without creating
#   routing waists.
#
# Required: set these properties in Vivado before launching impl_3:
#   set_property STEPS.PLACE_DESIGN.ARGS.DIRECTIVE AltSpreadLogic_medium \
#     [get_runs impl_3]
#   set_property STEPS.ROUTE_DESIGN.ARGS.DIRECTIVE \
#     AggressiveCongestionOptimization [get_runs impl_3]
#
# Gemmini: near clock root (X4Y7), away from MIG (Y11+)
# fmxcvu19p-additional.sdc creates pblock_gemmini0 and assigns cells, and
# also resize_pblock -adds its own generic default range (X0Y2:X3Y5).
# resize_pblock -add is cumulative, not a replace, so without removing that
# default first, pblock_gemmini0 would span the disjoint UNION of the
# generic range and this config's tuned range below, spreading gemmini
# across two unconnected chip regions and adding extra placement/routing
# pressure on top of the VRF congestion this section is designed to avoid.
set_property CONTAIN_ROUTING false [get_pblocks pblock_gemmini0]

########################################################################################
# pblock_vu/vfu/vmu A/B test, Test A arm (pblock active)
# Moved here from the shared fmxcvu19p-additional.sdc 2026.07.10: that file is
# parsed by Vivado's restricted XDC constraint interpreter, which doesn't support
# Tcl control-flow ([Designutils 20-1307] "Command 'if' is not supported in the
# xdc constraint file"), so the previous `if {[info exists bitname] ...}` gate
# there could not work. This file is already scoped to this one CONFIG by
# prologue.tcl's per-bitname `$bitname.sdc` glob, so no conditional is needed --
# unconditional here means "included" for this CONFIG, and simply absent from
# VelaSaturnRocket1CoreDedicatedGemminiABTestNoPblock.sdc means "not included"
# for that one. Result on this arm (SLR2/SLR1): WNS=-3.879/TNS=-147405, route legal.
########################################################################################
create_pblock pblock_vu
add_cells_to_pblock pblock_vu [get_cells -quiet [list chiptop0/system/tile_prci_domain/element_reset_domain_rockettile/vector_unit/vu]]
resize_pblock pblock_vu -add {SLR2}
set_property CONTAIN_ROUTING false [get_pblocks pblock_vu]

create_pblock pblock_vfu
add_cells_to_pblock pblock_vfu [get_cells -quiet [list chiptop0/system/tile_prci_domain/element_reset_domain_rockettile/vector_unit/vfu]]
resize_pblock pblock_vfu -add {SLR2}
set_property CONTAIN_ROUTING false [get_pblocks pblock_vfu]

create_pblock pblock_vmu
add_cells_to_pblock pblock_vmu [get_cells -quiet [list chiptop0/system/tile_prci_domain/element_reset_domain_rockettile/vector_unit/vmu]]
resize_pblock pblock_vmu -add {SLR1}
set_property CONTAIN_ROUTING false [get_pblocks pblock_vmu]
