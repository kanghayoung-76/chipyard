...
object FPGAGemminiConfigs {
  ...
  val fpgaConfig = GemminiConfigs.defaultConfig.copy(
    ...
    mesh_output_delay = 2,
    acc_scale_internal_latency = 2,  // 2 RegNext stages inside scale_func; delay in.valid to match
...
// Mirror of chipyard.config.WithMultiRoCCGemmini but uses velaNPU.GemminiArrayConfig
// (chipyard's version imports gemmini._, so it cannot accept velaNPU configs)
class WithMultiRoCCFPGAGemmini[T <: Data : Arithmetic, U <: Data, V <: Data](
  harts: Int*)(gemminiConfig: GemminiArrayConfig[T,U,V] = FPGAGemminiConfigs.fpgaConfig
) extends Config((site, here, up) => {
