...
import sifive.fpgashells.devices.xilinx.profmxcvu19pmig.{ProfmXCVU19PMIGPads, ProfmXCVU19PMIGParams, ProfmXCVU19PMIG}

class SysClock2FMXCVU19PPlacedOverlay(val shell: FMXCVU19PShellBasicOverlays, name: String, val designInput: ClockInputDesignInput, val shellInput: ClockInputShellInput)
...
class SysClock2FMXCVU19PShellPlacer(shell: FMXCVU19PShellBasicOverlays, val shellInput: ClockInputShellInput)(implicit val valName: ValName)
  extends ClockInputShellPlacer[FMXCVU19PShellBasicOverlays]
{
    def place(designInput: ClockInputDesignInput) = new SysClock2FMXCVU19PPlacedOverlay(shell, valName.name, designInput, shellInput)
}

case object FMXCVU19PDDR2Size extends Field[BigInt](0x40000000L * 2) // 2GB
class DDR2FMXCVU19PPlacedOverlay(val shell: FMXCVU19PFPGATestHarness, name: String, val designInput: DDRDesignInput, val shellInput: DDRShellInput)
  extends DDRPlacedOverlay[ProfmXCVU19PMIGPads](name, designInput, shellInput)
{
  val size = p(FMXCVU19PDDRSize)

  val migParams = ProfmXCVU19PMIGParams(address = AddressSet.misaligned(di.baseAddress, size))
  val mig = LazyModule(new ProfmXCVU19PMIG(migParams))
  ...
  def ioFactory = new ProfmXCVU19PMIGPads(size)
  ...
  shell { InModuleBody {
    require (shell.sys_clock2.get.isDefined, "Use of DDRFMXCVU19POverlay depends on SysClock2FMXCVU19POverlay")
...
class DDR2FMXCVU19PShellPlacer(shell: FMXCVU19PFPGATestHarness, val shellInput: DDRShellInput)(implicit val valName: ValName)
  extends DDRShellPlacer[FMXCVU19PFPGATestHarness] {
  def place(designInput: DDRDesignInput) = new DDR2FMXCVU19PPlacedOverlay(shell, valName.name, designInput, shellInput)
}
