...
class ScalePipe[T <: Data, U <: Data](t: T, mvin_scale_args: ScaleArguments[T, U]) extends Module {
  ...
  val internal_latency = mvin_scale_args.internal_latency
  val scaled_data = mvin_scale_args.scale_func(io.in.bits.data, io.in.bits.scale.asTypeOf(u))
  val in_delayed = Pipe(io.in, internal_latency)
  val out = WireInit(in_delayed)
  out.bits.data := scaled_data
  io.out := Pipe(out, latency - internal_latency)
}
...
class VectorScalarMultiplier[T <: Data, U <: Data, Tag <: Data](
  ...
  val (u, num_scale_units, always_identity) = mvin_scale_args match {
    case Some(ScaleArguments(_, _, multiplicand_t, num_scale_units, _, _, _)) => (multiplicand_t, num_scale_units, false)
  ...
  val latency = mvin_scale_args match {
    case Some(ScaleArguments(_, latency, _, _, _, _, _)) => latency
    case None => 0
  }
  ...
  if (num_scale_units == -1) {
    ...
    pipe.io.in.bits.out := (mvin_scale_args match {
      case Some(ScaleArguments(mvin_scale_func, _, multiplicand_t, _, _, _, _)) =>
