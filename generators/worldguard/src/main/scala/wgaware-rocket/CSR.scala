...
class WGCSRFile(
  ...
  if (usingHypervisor) {
    read_mapping += WGCSRs.mtinst -> read_mtinst
  ...
  when (csr_wen) {
    ...
    if (usingHypervisor) {
      ...
      when(decoded_addr(WGCSRs.mtinst)) { reg_mtinst_read_pseudo := write_mhtinst_read_pseudo }
      when(decoded_addr(WGCSRs.htinst)) { reg_htinst_read_pseudo := write_mhtinst_read_pseudo }
