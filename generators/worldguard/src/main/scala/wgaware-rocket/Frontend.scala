...
trait HasWGICacheFrontend extends CanHaveWGPTW { this: BaseTile =>
  val module: HasWGICacheFrontendModule
  val frontend = LazyModule(new WGFrontend(tileParams.icache.get, tileId))
...
trait HasWGICacheFrontendModule extends CanHaveWGPTWModule {
  val outer: HasWGICacheFrontend
  ptwPorts += outer.frontend.module.io.ptw
}

class WGFrontend(val icacheParams: ICacheParams, tileId: Int)(implicit p: Parameters) extends LazyModule {
  lazy val module = new WGFrontendModule(this)
  val icache = LazyModule(new WGICache(icacheParams, tileId))
  val masterNode = icache.masterNode
  val slaveNode = icache.slaveNode
  val resetVectorSinkNode = BundleBridgeSink[UInt](Some(() => UInt(masterNode.edges.out.head.bundle.addressBits.W)))
}

class WGFrontendIO(implicit p: Parameters) extends CoreBundle()(p) {
...
class WGFrontendBundle(val outer: WGFrontend) extends CoreBundle()(outer.p) {
  val cpu = Flipped(new WGFrontendIO())
  val ptw = new WGTLBPTWIO()
  val errors = new ICacheErrors
}

class WGFrontendModule(outer: WGFrontend) extends LazyModuleImp(outer)
    with HasRocketCoreParameters
    with HasL1ICacheParameters {
  val io = IO(new WGFrontendBundle(outer))
  ...
  val tlb = Module(new WGTLB(true, log2Ceil(fetchBytes), TLBConfig(nTLBSets, nTLBWays, outer.icacheParams.nTLBBasePageSectors, outer.icacheParams.nTLBSuperpages)))
  ...
  tlb.io.req.bits.wid := io.cpu.wid
