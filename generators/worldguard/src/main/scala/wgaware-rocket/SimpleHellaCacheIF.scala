 ...
 * completed.
 */
class WGSimpleHellaCacheIFReplayQueue(depth: Int)
    (implicit val p: Parameters) extends Module
    with HasL1HellaCacheParameters {
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new WGHellaCacheReq))
    val nack = Flipped(Valid(Bits(coreParams.dcacheReqTagBits.W)))
    val resp = Flipped(Valid(new HellaCacheResp))
    val replay = Decoupled(new WGHellaCacheReq)
  })
  ...
  val reqs = Reg(Vec(depth, new WGHellaCacheReq))
...
class WGSimpleHellaCacheIF(implicit p: Parameters) extends Module
{
  val io = IO(new Bundle {
    val requestor = Flipped(new WGHellaCacheIO())
    val cache = new WGHellaCacheIO
  })
  io <> DontCare

  /* [1.13] upstream 이 리플레이 큐 깊이를 2->3 으로 올렸다 - WG 큐에도 반영 */
  val replayq = Module(new WGSimpleHellaCacheIFReplayQueue(3))
  val req_arb = Module(new Arbiter(new WGHellaCacheReq, 2))
