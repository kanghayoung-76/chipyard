...
class WGTLBReq(lgMaxSize: Int)(implicit p: Parameters) extends CoreBundle()(p) {
...
class WGTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  val io = IO(new Bundle {
    /** request from Core */
    val req = Flipped(Decoupled(new WGTLBReq(lgMaxSize)))
    /** response to Core */
  ...
  println(s"[tlb] sectored entry : ${cfg.nSets} sets  ${cfg.nWays} ways ${cfg.nSectors}")
  println(s"[tlb] superpage entry: ${cfg.nSuperpageEntries} entries")
  println(s"[tlb] pageGranularityPMPs: ${pageGranularityPMPs}")
  ...
  val r_wid = Reg(UInt(log2Ceil(p(NWorlds)).W))  // It's not a TLB entry but for timing
  ...
  io.ptw.req.bits.bits.wid := r_wid
