 ...
 * Regenerated from this chipyard's rocket-chip RocketTile.scala (rename fork).
 * NOTE: unlike the Vyond original, this chipyard's rocket-chip has no
 * freechips.rocketchip.trace package, so all trace-encoder support (which WG
 * itself never modified, just inherited from a newer stock RocketTile.scala)
 * has been dropped here to match.
 */
...
import freechips.rocketchip.prci.{ClockCrossingType, RationalCrossing, ClockSinkParameters}
...
class WGRocketTile private(
    ...
    with HasWGLazyRoCC  // implies CanHaveSharedFPU with CanHavePTW with HasWGHellaCache
...
class WGRocketTileModuleImp(outer: WGRocketTile) extends BaseTileModuleImp(outer)
  ...
  // The WG fork does not support the vector unit sharing the WG D$ (its
  // io.dmem is a plain, WID-less HellaCacheIO). WG configs don't use vector,
  // so this is enforced explicitly rather than silently mis-wired.
  outer.vector_unit foreach { v =>
    require(!outer.rocketParams.core.vector.get.useDCache,
      "WGRocketTile does not support vector units sharing the WG D$")
