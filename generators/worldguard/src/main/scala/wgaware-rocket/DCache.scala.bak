...
class WGDCacheTLBPort(implicit p: Parameters) extends CoreBundle()(p) {
  val req = Flipped(Decoupled(new WGTLBReq(coreDataBytes.log2)))
  val s1_resp = Output(new TLBResp)
  val s2_kill = Input(Bool())
}

class WGDCacheMetadataReq()(implicit p: Parameters) extends L1HellaCacheBundle()(p) {
  val write = Bool()
  val addr = UInt(vaddrBitsExtended.W)
  val idx = UInt(idxBits.W)
  val way_en = UInt(nWays.W)
  val data = UInt(cacheParams.tagCode.width(new WGL1Metadata().getWidth).W)
}

class WGDCache(widWidth: Int, staticIdForMetadataUseOnly: Int, val crossing: ClockCrossingType)(implicit p: Parameters) extends WGHellaCache(staticIdForMetadataUseOnly)(p) {
  override lazy val module = new WGDCacheModule(widWidth, this)
}

class WGDCacheModule(widWidth: Int, outer: WGDCache) extends WGHellaCacheModule(outer) {
  /* [1.13] tlb_port 는 별도 IO 가 아니라 WGHellaCacheBundle(io.tlb_port)로 이동했다 */
  ...
  val tlb = Module(new WGTLB(false, log2Ceil(coreDataBytes), TLBConfig(nTLBSets, nTLBWays, cacheParams.nTLBBasePageSectors, cacheParams.nTLBSuperpages)))
  val pma_checker = Module(new WGTLB(false, log2Ceil(coreDataBytes), TLBConfig(nTLBSets, nTLBWays, cacheParams.nTLBBasePageSectors, cacheParams.nTLBSuperpages)) with InlineInstance)
  ...
  /** Metadata Arbiter:
    ...
    */
  val metaArb = Module(new Arbiter(new WGDCacheMetadataReq(), 8) with InlineInstance)
  ...
  val uncachedReqs = Reg(Vec(maxUncachedInFlight, new WGHellaCacheReq))
  val uncachedResp = WireInit(new WGHellaCacheReq, DontCare)
  ...
  pma_checker.io.req.bits.wid := s1_req.wid
  ...
  val (s1_hit_way, s1_hit_state, s1_meta, s1_hit_way_wid) =
    if (usingDataScratchpad) {
      ...
      val dummyMeta = WGL1Metadata(0.U, ClientMetadata.onReset, ~0.U(widWidth.W))
      (inScratchpad, hitState, Seq(tECC.encode(dummyMeta.asUInt)), 0.U/*dummy*/)
    } else {
      ...
      val s1_meta_uncorrected = s1_meta.map(tECC.decode(_).uncorrected.asTypeOf(new WGL1Metadata))
      ...
      val s1_meta_hit_way_wid = s1_meta_uncorrected.map(r => r.coh.isValid() && r.tag === s1_tag ).asUInt

      //(s1_meta_hit_way, s1_meta_hit_state, s1_meta)
      (s1_meta_hit_way, s1_meta_hit_state, s1_meta, s1_meta_hit_way_wid)
    }
  ...
  val s2_meta_corrected = s1_meta_decoded.map(m => RegEnable(m.corrected, s1_meta_clk_en).asTypeOf(new WGL1Metadata))
  ...
  // replacement of mismatched wid
  val s2_hit_way_wid = RegEnable(s1_hit_way_wid, s1_valid_not_nacked)
  ...
  //------------------------------------------------------
  // hit both tag/wid  | hit tag | operation
  //------------------------------------------------------
  //                1  |       x | take a path to hit
  //                0  |       1 | replace s2_hit_way_wid 
  //                0  |       0 | normal miss
  //------------------------------------------------------
  // replacement of mismatched wid
  //val s2_victim_or_hit_way = Mux(s2_hit_valid, s2_hit_way, s2_victim_way)
  val s2_victim_or_hit_way = Mux(s2_hit_valid, s2_hit_way, Mux(s2_hit_way_wid =/= 0.U, s2_hit_way_wid, s2_victim_way))
  ...
  val s2_replace_hit_way_wid = !s2_hit_valid && (s2_hit_way_wid =/= 0.U)
  val s2_victim_tag = Mux(s2_valid_data_error || s2_valid_flush_line,
    s2_req.addr(paddrBits-1, tagLSB),
    Mux1H(Mux(s2_replace_hit_way_wid, s2_hit_way_wid, s2_victim_way), s2_meta_corrected).tag)
  val s2_victim_wid = Mux(s2_valid_data_error || s2_valid_flush_line, s2_req.wid,
    Mux1H(Mux(s2_replace_hit_way_wid, s2_hit_way_wid, s2_victim_way), s2_meta_corrected).wid)
  
  // replacement of mismatched wid
  //val s2_victim_state = Mux(s2_hit_valid, s2_hit_state, Mux1H(s2_victim_way, s2_meta_corrected).coh)
  val s2_victim_state = Mux(s2_hit_valid, s2_hit_state, Mux1H(Mux(s2_hit_way_wid > 0.U, s2_hit_way_wid, s2_victim_way), s2_meta_corrected).coh)
  ...
  // Drive wid
  tl_out_a.bits.user.lift(WGTLCustomFieldKey).foreach { x =>
    x.wid := s2_req.wid
  }
  ...
  if (!usingDataScratchpad) {
    ...
    tl_out_c.bits.user.lift(WGTLCustomFieldKey).foreach { x =>
      x.wid := s2_victim_wid
    }
  }
  ...
  metaArb.io.in(4).bits.data := tECC.encode(WGL1Metadata(tl_out_c.bits.address >> tagLSB, newCoh, ~0.U(widWidth.W)).asUInt)  // Regarding wid, there is only case of toN
  ...
  metaArb.io.in(0).bits.data := tECC.encode(WGL1Metadata(0.U, ClientMetadata.onReset, ~0.U(widWidth.W)).asUInt)
  when (resetting) {
    flushCounter := flushCounterNext
    when (flushDone) {
      resetting := false.B
