...
trait HasWGRocketCoreIO extends HasRocketCoreParameters {
  implicit val p: Parameters
  def nTotalRoCCCSRs: Int
  val io = IO(new CoreBundle()(p) {
    val hartid = Input(UInt(hartIdLen.W))
    val reset_vector = Input(UInt(resetVectorLen.W))
    val interrupts = Input(new CoreInterrupts(tileParams.asInstanceOf[WGRocketTileParams].beuAddr.isDefined))
    val imem  = new WGFrontendIO
    val dmem = new WGHellaCacheIO
    val ptw = Flipped(new DatapathPTWIO())
    val fpu = Flipped(new FPUCoreIO())
    val rocc = Flipped(new WGRoCCCoreIO(nTotalRoCCCSRs))
...
class WGRocket(tile: WGRocketTile)(implicit p: Parameters) extends CoreModule()(p)
    with HasRocketCoreParameters
    with HasWGRocketCoreIO {
  ...
  val csr = Module(new WGCSRFile(perfEvents, coreParams.customCSRs.decls, tile.roccCSRs.flatten, tile.rocketParams.beuAddr.isDefined))
  ...
  val id_illegal_insn = !id_ctrl.legal ||
    ...
    id_csr_en && csr.io.decode(0).wg_access_illegal
  ...
  io.dmem.req.bits.wid := csr.io.wid
  ...
  io.rocc.cmd.bits.wid := csr.io.wid
