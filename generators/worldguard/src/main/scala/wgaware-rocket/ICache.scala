...
class WGICacheResp(outer: WGICache) extends Bundle {
  /** data to CPU. */
...
class WGICacheBundle(val outer: WGICache) extends CoreBundle()(outer.p) {
  ...
  /** response to CPU. */
  val resp = Valid(new WGICacheResp(outer))
  ...
  val wid = Input(UInt(log2Ceil(outer.p(NWorlds)).W))
}

class WGICache(val icacheParams: ICacheParams, val staticIdForMetadataUseOnly: Int)(implicit p: Parameters) extends LazyModule {
  lazy val module = new WGICacheModule(this)
...
class WGICacheModule(outer: WGICache) extends LazyModuleImp(outer)
    with HasL1ICacheParameters {
  override val cacheParams = outer.icacheParams // Use the local parameters

  val widWidths = log2Ceil(outer.p(NWorlds))
  /** IO between Core and ICache. */
  val io = IO(new WGICacheBundle(outer))
  ...
  assert(!scratchpadOn, s"WorldGuard does not support scratch pad yet")
  ...
  val s1_tag_only_hit = s1_tag_only_hits.reduce(_||_) // || Mux(s1_slaveValid, true.B, addrMaybeInScratchpad(io.s1_paddr))  <-- TODO: should it be considered?
  dontTouch(s1_hit)
  ...
  val victim_way = if (isDM) 0.U else {
  ...
  val repl_way = Mux(refill_dueto_wid_miss, refill_wid_miss_way, victim_way)
  dontTouch(repl_way)
  dontTouch(victim_way)
 ...
 * */
  val tag_array  = DescribedSRAM(
    ...
    size = nSets,
    data = Vec(nWays, UInt(tECC.width(1 + tagBits + widWidths).W))
  )
  ...
  when (refill_done) {
    ...
    val enc_tag = tECC.encode(Cat(refillError, refill_tag, refill_wid))
  ...
  for (i <- 0 until nWays) {
    ...
    /** [[tl_error]] ECC error bit.
      ...
      */
    val (tl_error, tag, wid) = Split(enc_tag.uncorrected, tagBits + widWidths, widWidths)
    dontTouch(tag)
    dontTouch(wid)
    val tagMatch = s1_vb && tag === s1_tag && wid === s1_wid
    val tagOnlyMatch = s1_vb && tag === s1_tag
    /** tag error happens. */
    ...
    s1_tag_only_hits(i) := tagOnlyMatch || scratchpadHit
  }
  ...
  // Drive wid
  tl_out.a.bits.user.lift(WGTLCustomFieldKey).foreach { x =>
    x.wid := refill_wid
    // FIXME: consider request from TLBPTWIO
  }
