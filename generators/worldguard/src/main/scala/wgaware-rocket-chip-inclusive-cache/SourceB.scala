...
class WGSourceBRequest(params: InclusiveCacheParameters, widBits: Int) extends InclusiveCacheBundle(params)
{
  val param   = UInt(3.W)
...
class WGSourceB(params: InclusiveCacheParameters, widBits: Int) extends Module
{
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new WGSourceBRequest(params, widBits)))
  ...
  if (params.firstLevel) {
    ...
    val wid = Mux(!busy, io.req.bits.wid, RegEnable(io.req.bits.wid, io.req.fire))
