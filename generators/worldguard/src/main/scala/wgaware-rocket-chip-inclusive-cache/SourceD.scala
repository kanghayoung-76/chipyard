...
class WGSourceDRequest(params: InclusiveCacheParameters, widBits: Int) extends FullRequest(params)
{
...
class WGSourceD(params: InclusiveCacheParameters, widBits: Int) extends Module
{
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new WGSourceDRequest(params, widBits)))
  ...
  //d.bits.user.lift(WGTLCustomFieldKey).foreach { x =>
  //  x.wid := s3_req.wid
  //}
  ...
  atomics.io.a.user.lift(WGTLCustomFieldKey).foreach { x =>
    x.wid := DontCare
  }
