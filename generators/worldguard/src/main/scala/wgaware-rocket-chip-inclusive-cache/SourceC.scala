...
class WGSourceCRequest(params: InclusiveCacheParameters, widBits: Int) extends InclusiveCacheBundle(params)
{
  val opcode = UInt(3.W)
  val param  = UInt(3.W)
...
class WGSourceC(params: InclusiveCacheParameters, widBits: Int) extends Module
{
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new WGSourceCRequest(params, widBits)))
  ...
  c.bits.user.lift(WGTLCustomFieldKey).foreach { x =>
    x.wid := s3_req.wid
  }
