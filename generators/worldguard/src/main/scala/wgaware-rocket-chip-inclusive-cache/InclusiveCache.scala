    ...
    extends LazyModule
{
  ...
  val rf = if (p(UseWGTLCustomField)) {Seq(WGTLCustomField(log2Ceil(p(NWorlds))))} else Seq()
  ...
  val node: TLAdapterNode = TLAdapterNode(
    clientFn  = { _ => TLClientPortParameters(Seq(TLClientParameters(
      ...
      supportsProbe = xfer)), requestFields= rf)
    },
    managerFn = { m => TLManagerPortParameters(
      ...
      minLatency = 2, responseFields = rf, requestKeys = rk)
    })

  val ctrls = control.map { c =>
    val nCtrls = if (c.bankedControl) p(SubsystemBankedCoherenceKey).nBanks else 1
    Seq.tabulate(nCtrls) { i => LazyModule(new WGInclusiveCacheControl(this,
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val mods = (node.in zip node.out) map { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val scheduler = Module(new WGInclusiveCacheBankScheduler(params, widBits)).suggestName("inclusive_cache_bank_sched")
      ...
      scheduler.io.req.bits.wid := 0.U // FIXME: Temporarily set wid to 0
