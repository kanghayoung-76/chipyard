...
class WGInclusiveCacheBankScheduler(params: InclusiveCacheParameters, widBits: Int) extends Module
{
  ...
  val sourceB = Module(new WGSourceB(params, widBits))
  val sourceC = Module(new WGSourceC(params, widBits))
  val sourceD = Module(new WGSourceD(params, widBits))
  val sourceE = Module(new WGSourceE(params, widBits))
  val sourceX = Module(new WGSourceX(params, widBits))
  ...
  val sinkA = Module(new WGSinkA(params, widBits))
  val sinkC = Module(new WGSinkC(params, widBits))
  val sinkD = Module(new SinkD(params))
  val sinkE = Module(new WGSinkE(params, widBits))
  val sinkX = Module(new WGSinkX(params, widBits))
  ...
  val directory = Module(new WGDirectory(params, widBits))
  val bankedStore = Module(new BankedStore(params))
  val requests = Module(new ListBuffer(ListBufferParameters(new WGQueuedRequest(params, widBits), 3*params.mshrs, params.secondary, false)))
  println(s"[l2cache] num mshrs: ${params.mshrs}")
  val mshrs = Seq.fill(params.mshrs) { Module(new WGMSHR(params, widBits)) }    // 7
  val abc_mshrs = mshrs.init.init                                    // [0:4]
  val bc_mshr = mshrs.init.last                                      // [5]
  val c_mshr = mshrs.last                                            // [6]
  val nestedwb = Wire(new WGNestedWriteback(params, widBits))
  ...
  dontTouch(schedule)  // NOTE: remove it after wg is stable later.
  dontTouch(scheduleTag)  // NOTE: remove it after wg is stable later.
  dontTouch(scheduleSet)  // NOTE: remove it after wg is stable later.
  dontTouch(mshr_selectOH)  // NOTE: remove it after wg is stable later.
  ...
  nestedwb.wid   := Mux(select_c, c_mshr.io.status.bits.wid, bc_mshr.io.status.bits.wid)
  nestedwb.b_toN       := select_bc && bc_mshr.io.schedule.bits.dir.valid && bc_mshr.io.schedule.bits.dir.bits.data.state === WGMetaData.INVALID
  nestedwb.b_toB       := select_bc && bc_mshr.io.schedule.bits.dir.valid && bc_mshr.io.schedule.bits.dir.bits.data.state === WGMetaData.BRANCH
  ...
  val request = Wire(Decoupled(new WGFullRequest(params, widBits)))
  dontTouch(request)  // NOTE: remove it after wg is stable later.
  ...
  mshrs.zipWithIndex.foreach { case (m, i) =>
    ...
    m.io.allocate.bits.viewAsSupertype(chiselTypeOf(requests.io.data)) := Mux(bypass, WireInit(new WGQueuedRequest(params, widBits), init = request.bits), requests.io.data)
    m.io.allocate.bits.set := m.io.status.bits.set
    m.io.allocate.bits.wid := m.io.status.bits.wid
  ...
  directory.io.read.bits.wid := Mux(mshr_uses_directory_for_lb, requests.io.data.wid, request.bits.wid)
  directory.io.read.bits.prio := Mux(mshr_uses_directory_for_lb, requests.io.data.prio, request.bits.prio)
  directory.io.read.bits.opcode := Mux(mshr_uses_directory_for_lb, requests.io.data.opcode, request.bits.opcode)
