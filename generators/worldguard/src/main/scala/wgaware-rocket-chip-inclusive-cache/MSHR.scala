...
class WGScheduleRequest(params: InclusiveCacheParameters, widBits: Int) extends InclusiveCacheBundle(params)
{
  val a = Valid(new WGSourceARequest(params, widBits))
  val b = Valid(new WGSourceBRequest(params, widBits))
  val c = Valid(new WGSourceCRequest(params, widBits))
  val d = Valid(new WGSourceDRequest(params, widBits))
  val e = Valid(new WGSourceERequest(params, widBits))
  val x = Valid(new WGSourceXRequest(params, widBits))
  val dir = Valid(new WGDirectoryWrite(params, widBits))
  val reload = Bool() // get next request via allocate (if any)
}

class WGMSHRStatus(params: InclusiveCacheParameters, widBits: Int) extends InclusiveCacheBundle(params)
{
...
class WGNestedWriteback(params: InclusiveCacheParameters, widBits: Int) extends InclusiveCacheBundle(params)
{
  val set = UInt(params.setBits.W)
  val tag = UInt(params.tagBits.W)
  val wid = UInt(widBits.W)
...
class WGMSHR(params: InclusiveCacheParameters, widBits: Int) extends Module
{
  val io = IO(new Bundle {
    val allocate  = Flipped(Valid(new WGAllocateRequest(params, widBits))) // refills MSHR for next cycle  // Input to indicate that this entry is allocated
    val directory = Flipped(Valid(new WGDirectoryResult(params, widBits))) // triggers schedule setup      // Input from directory with a result of tag lookup
    val status    = Valid(new WGMSHRStatus(params, widBits))                                    // Status of the current request
    val schedule  = Decoupled(new WGScheduleRequest(params, widBits))                           // Output to indicate one of sink or source is valid
    ...
    val nestedwb  = Flipped(new WGNestedWriteback(params, widBits))
  })

  val request_valid = RegInit(false.B)
  val request = Reg(new WGFullRequest(params, widBits))
  val meta_valid = RegInit(false.B)
  val meta = Reg(new WGDirectoryResult(params, widBits))

  // Define which states are valid
  when (meta_valid) {
    // replacement of mismatched wid
    // TODO: add asserts related to wid if any
  ...
  val s_release_wid    = RegInit(true.B) // To guarantee release first than acquire in case tag hit wid miss  
  ...
  io.status.bits.wid    := request.wid
  ...
  when (w_releaseack) {
    s_release_wid := true.B // Now a.valid can be asserted.
  }

  io.schedule.bits.a.valid := !s_acquire && s_release && s_pprobe && s_release_wid
  ...
  when (meta_valid && meta.onlyTagHit && !meta.hit) {
    final_meta_writeback.wid := request.wid
  }
  ...
  when (bad_grant) {
    when (meta.hit) {
      ...
      final_meta_writeback.clients := meta.clients & ~probes_toN
      final_meta_writeback.wid     := meta.wid
    } .otherwise {
      // failed N -> (T or B)
      final_meta_writeback.hit     := false.B
      final_meta_writeback.dirty   := false.B
      ...
      final_meta_writeback.wid     := 0.U // TODO: Is it safe?
    }
  }

  val invalid = Wire(new WGDirectoryEntry(params, widBits))
  invalid.dirty   := false.B
  invalid.state   := INVALID
  invalid.clients := 0.U
  invalid.tag     := 0.U
  invalid.wid     := 0.U  // TODO: Is it safe?
  ...
  io.schedule.bits.a.bits.wid     := request.wid
  ...
  io.schedule.bits.b.bits.wid     := request.wid
  ...
  io.schedule.bits.d.bits.wid     := request.wid 
  io.schedule.bits.e.bits.sink    := sink
  io.schedule.bits.e.bits.wid     := request.wid
  io.schedule.bits.x.bits.fail    := false.B
  io.schedule.bits.x.bits.wid     := meta.wid
  ...
  val allocate_as_full = WireInit(new WGFullRequest(params, widBits), init = io.allocate.bits)
