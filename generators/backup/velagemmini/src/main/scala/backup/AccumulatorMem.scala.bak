...
class AccumulatorMem[T <: Data, U <: Data](
  ...
  // Debug probe (Verilator simulation only — printf not visible on FPGA UART)
  // DP3-ACC: fires once per output row written into the accumulator; show first 4 columns
  when (!reset.asBool && io.write.fire) {
    printf("[DP3-ACC] addr=0x%x acc=%d data=%d_%d_%d_%d\n",
      io.write.bits.addr,
      io.write.bits.acc,
      io.write.bits.data(0)(0).asUInt.asSInt,
      io.write.bits.data(1)(0).asUInt.asSInt,
      io.write.bits.data(2)(0).asUInt.asSInt,
      io.write.bits.data(3)(0).asUInt.asSInt)
  }
}
