...
class StoreController[T <: Data : Arithmetic, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V],
  ...
  // DP4-CMD: mvout store command accepted
  when (!reset.asBool && DoStore && cmd_tracker.io.alloc.fire()) {
    printf("[DP4-CMD] vaddr=0x%x laddr=0x%x rows=%d cols=%d\n",
      vaddr, localaddr.asUInt, rows, cols)
  }
  // DP4-DMA: each DMA write request to DRAM fires (one per output row)
  when (!reset.asBool && io.dma.req.fire) {
    printf("[DP4-DMA] vaddr=0x%x laddr=0x%x\n",
      io.dma.req.bits.vaddr, io.dma.req.bits.laddr.asUInt)
  }
}
