...
class LoadController[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V], coreMaxAddrBits: Int,
  ...
  when (!reset.asBool && DoLoad && cmd_tracker.io.alloc.fire()) {
    printf("[DP1-CMD] vaddr=0x%x spad=0x%x rows=%d cols=%d\n",
      vaddr, localaddr.asUInt, rows, cols)
  }
  when (!reset.asBool && io.dma.resp.fire) {
    printf("[DP1-ROW] cmd_id=%d bytes=%d\n",
      io.dma.resp.bits.cmd_id, io.dma.resp.bits.bytesRead)
  }
