...
//preload_zero일 때 padding 추가해서 pipeline 맞춰주기
 ...
 // Performance counter
  ...
  // Debug probes (Verilator simulation only — printf not visible on FPGA UART)
  // DP2-PRE: preload command accepted (bc_address_place=0 when DoPreloads(0))
  when (!reset.asBool && control_state === waiting_for_cmd &&
        DoPreloads(0) && cmd.valid(1) && (raw_hazards_are_impossible.B || !raw_hazard_pre)) {
    printf("[DP2-PRE] b_spad=0x%x c_acc=0x%x b_rows=%d b_cols=%d\n",
      rs1s(0)(31, 0), rs2s(0)(31, 0), b_rows, b_cols)
  }
  // DP2-COMP: compute command accepted (covers single-mul and overlap mul+pre)
  when (!reset.asBool && control_state === waiting_for_cmd && DoComputes(0)) {
    printf("[DP2-COMP] a_spad=0x%x d_spad=0x%x a_rows=%d a_cols=%d\n",
      rs1s(0)(31, 0), rs2s(0)(31, 0), a_rows, a_cols)
  }
  // DP2-MESH: each cycle the mesh receives a valid A-row (fires once per input row)
  when (!reset.asBool && cntl_valid && wontolic.io.a.valid) {
    printf("[DP2-MESH] a=%d_%d_%d_%d b=%d_%d_%d_%d\n",
      dataA(0).asUInt.asSInt, dataA(1).asUInt.asSInt, dataA(2).asUInt.asSInt, dataA(3).asUInt.asSInt,
      dataB(0).asUInt.asSInt, dataB(1).asUInt.asSInt, dataB(2).asUInt.asSInt, dataB(3).asUInt.asSInt)
  }
