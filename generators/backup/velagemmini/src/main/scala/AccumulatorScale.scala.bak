...
class AccumulatorScale[T <: Data, U <: Data](
  ...
  internal_latency: Int = 0)(implicit ev: Arithmetic[T]) extends Module {
  ...
  if (num_scale_units == -1) {
    ...
    // Delay in.valid by internal_latency cycles so that activated_data (which
    // depends on RegNext stages inside scale_func) is stable when Pipeline latches.
    // For a purely combinatorial scale_func, internal_latency=0 (default) is correct.
    in.valid := ShiftRegister(io.in.valid, internal_latency)
    io.in.ready := in.ready
    in.bits.resp := io.in.bits.acc_read_resp
    in.bits.full_data := activated_data
    in.bits.resp.data := clipped_data

    // Pipeline depth reduced by internal_latency because those cycles are already
    // consumed waiting for activated_data to be correct.
    val pipe_out = Pipeline(in, latency - internal_latency)
