...
class RegisterFile(reads: Seq[Int], maskReads: Seq[Int], pipeWrites: Int, llWrites: Int)(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
  ...
  // Support 1, 2, 4, and 8 banks for the VRF. bankId/vrfBankBits/RegisterReadXbar
  // are all derived generically from log2Ceil(nBanks), so no other change is
  // needed to support 8 -- this only relaxes the power-of-2 sanity check.
