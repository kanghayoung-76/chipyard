...
import gemmininpu.Arithmetic.SIntArithmetic
import hardfloat._
  ...
  val defaultConfig = GemminiArrayConfig[SInt, Float, Float](
    ...
    mvin_scale_args = Some(ScaleArguments(
      (t: SInt, f: Float) => {
        ...
        val t_rec_reg = RegNext(in_to_rec_fn.io.out)
        val f_rec_reg = RegNext(f_rec)
        ...
        muladder.io.a := t_rec_reg
        muladder.io.b := f_rec_reg
        muladder.io.c := 0.U

        // pipeline register 2: after MulAddRecFN
        val mul_out_reg = RegNext(muladder.io.out)

        val rec_fn_to_in = Module(new RecFNToIN(f.expWidth, f.sigWidth, t.getWidth))
        rec_fn_to_in.io.in := mul_out_reg
