...
class LoopConvState(val block_size: Int, val large_iterator_bitwidth: Int, val small_iterator_bitwidth: Int, val tiny_iterator_bitwidth: Int, val coreMaxAddrBits: Int, val max_addr: Int, val max_acc_addr: Int) extends Bundle {
  ...
  // Added by ETRI hjkwon 20260520
  val cached_irows_without_dilation = UInt(small_iterator_bitwidth.W)
  val cached_icols_without_dilation = UInt(small_iterator_bitwidth.W)
  // End Added by ETRI hjkwon 20260520
  ...
  def derived_params(dummy: Int=0): LoopConvDerivedParams = {
    ...
    /*Changed by ETRI hjkwon 20260520
    ...
    // End Changed by ETRI hjkwon 20260520 */
  // Added by ETRI hjkwon 20260520
    val irows_without_dilation = cached_irows_without_dilation
    val icols_without_dilation = cached_icols_without_dilation
  // End Added by ETRI hjkwon 20260520
...
class LoopConv (block_size: Int, coreMaxAddrBits: Int, reservation_station_size: Int, max_lds: Int, max_exs: Int, max_sts: Int,
  ...
  // Added by ETRI hjkwon 20260520
        val lc_krows = loop_being_configured.inner_bounds.krows
        val lc_kcols = loop_being_configured.inner_bounds.kcols
        val lc_orows = loop_being_configured.inner_bounds.orows
        val lc_ocols = loop_being_configured.inner_bounds.ocols
        val lc_stride = loop_being_configured.outer_bounds.stride
        val lc_kd     = loop_being_configured.outer_bounds.kernel_dilation
        val lc_dilated_krows = lc_krows + (lc_kd - 1.U) * (lc_krows - 1.U)
        val lc_dilated_kcols = lc_kcols + (lc_kd - 1.U) * (lc_kcols - 1.U)
        loop_being_configured.cached_irows_without_dilation :=
          (lc_orows * lc_stride +& lc_dilated_krows -& 1.U)(small_iterator_bitwidth-1, 0)
        loop_being_configured.cached_icols_without_dilation :=
          (lc_ocols * lc_stride +& lc_dilated_kcols -& 1.U)(small_iterator_bitwidth-1, 0)
  // End Added by ETRI hjkwon 20260520
