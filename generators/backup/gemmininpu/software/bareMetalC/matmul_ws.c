...
int main() {
  ...
  for (int activation = AINIT; activation <= RELU; ++activation) {
    ...
    for (acc_scale_t scale = SINIT; scale <= 12; scale += 4) {
      ...
      // Force COW pages for C to be resolved to writable physical pages before
      // gemmini mvout. Without this, gemmini's PTW sees a read-only zero page
      // and the DMA write silently fails on Linux (no mlockall in BAREMETAL mode).
      memset(C, 0, sizeof(C));
      ...
      for (size_t n = 0; n < N; ++n) {
        for (size_t i = 0; i < DIM; ++i) {
          for (size_t j = 0; j < DIM; ++j) {
            A[n][i][j] = (i % 4) + 1;   // row-periodic: 1,2,3,4
            B[n][i][j] = 1;              // all ones
            D[n][i][j] = (j % 4) + 1;   // col-periodic: 1,2,3,4
          }
        }
      }
